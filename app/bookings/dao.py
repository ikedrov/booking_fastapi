...
class BookingDAO(BaseDAO):
    ...
    async def add(cls, user_id: int, room_id: int, date_from: date, date_to: date):
        ...
        WHERE rooms.id = 1
        GROUP_BY rooms.quantity, booked_rooms.room.id
        """
        if not isinstance(user_id, int) or not isinstance(room_id, int):
            return None
        try:
    ...
    @classmethod
    async def find_booking_for_user(cls, user_id: int):
        async with async_session_maker() as session:
            query = (
