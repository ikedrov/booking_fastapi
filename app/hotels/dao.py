    ...
    async def find_all(cls, location: str, date_from: date, date_to: date):
        async with async_session_maker() as session:
            """
                   ...
                   """
            booked_rooms = (select(Bookings.room_id, func.count(Bookings.room_id)
                                   .label('rooms_booked')).select_from(Bookings)
                            .where(
                or_(
                    and_(
                        Bookings.date_from >= Bookings.date_from,
                        Bookings.date_from <= Bookings.date_to
                    ),
                    and_(
                        Bookings.date_from <= Bookings.date_from,
                        Bookings.date_to > Bookings.date_from
                    )
                )
            ...
            booked_hotels = (
                select(Rooms.hotel_id, func.sum(Rooms.quantity - func.coalesce(booked_rooms.c.rooms_booked, 0)).label('rooms_left'))
                .select_from(Rooms)
